...
def create_LinearStepDevice(dwmin):
    device = LinearStepDevice()

    device.dw_min = dwmin
    device.dw_min_dtod = 0.0
    device.dw_min_std = 0.0

    device.up_down = 0.0
    device.up_down_dtod = 0.0

    device.w_min = -0.6
    device.w_max = 0.6
    device.w_min_dtod = 0.0
    device.w_max_dtod = 0.0
    
    return device



def create_analog_network(input_size, hidden_sizes, output_size, dwmin):
    ...
    model = AnalogSequential(
        AnalogLinear(input_size, hidden_sizes[0], True,
                     rpu_config=SingleRPUConfig(device=create_LinearStepDevice(dwmin))),
        nn.Sigmoid(),
...
def main():
    ...
    dwmin_list = [0.001, 0.002, 0.003]

    for dwmin in dwmin_list:
        # Prepare the model.
        model = create_analog_network(INPUT_SIZE, HIDDEN_SIZES, OUTPUT_SIZE, dwmin)
